...
def _clear_cache():
    """Hapus cache baca setelah ada operasi tulis, supaya data yang ditampilkan tetap fresh."""
    st.cache_data.clear()
...
def _get_sheet(name: str):
    """Ambil worksheet, buat otomatis kalau belum ada. Di-cache per nama sheet
    supaya tidak buka koneksi API berulang setiap kali fungsi load/save dipanggil
    (penyebab utama error 429 rate limit)."""
    ss = _get_spreadsheet()
    try:
